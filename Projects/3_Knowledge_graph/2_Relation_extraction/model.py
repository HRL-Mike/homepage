...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.cls_dropout = nn.Dropout(0.1)  # dropout on CLS transformed token embedding  防止过拟合
        self.ent_dropout = nn.Dropout(0.1)  # dropout on average entity embedding  防止过拟合
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, e1_mask=None, e2_mask=None, labels=None,
        ...
        # outputs[1]是整个输入的表达, the overall context of the sequence
        ...
        # outputs[0]是每个token的表示, at token-level
        ...
        pooled_output = torch.cat([context, e1_h, e2_h], dim=-1)  # 拼接三个向量 (CLS, 头实体, 尾实体)
